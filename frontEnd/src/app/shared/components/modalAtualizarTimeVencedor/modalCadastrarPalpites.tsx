...
export interface IModalVencedor{
    openModal: boolean;
    closeModal: () => void;
    onSuccess: () => void;
...
export default function ModalVencedor({
    closeModal,
    openModal,
    onSuccess,
    // dadosJogador,
    ...
    const AtualizarVencedor = async () => {
        try {
            const response = await ApiServiceBolao.ControllerPartidas.AtualizarVencedor({
                idPartida: opcaoSelecionada.idPartida,
                vencedorIdTime: Number(palpiteSelecionado.id)
            })
            onSuccess()
            toast.success("Time vencedor cadastrado com sucesso!")
